#include <stdint.h>
#include <stdio.h>
#include <parsing.h> // Assuming this includes parse_buffer() and interpolation()
...
int main(int argc, char** argv) {
    ...
    uint8_t sbus_packet[25]; // Size set to 25
    uint16_t *channel;       // Pointer to store parsed channel values

    // Open the serial ports for reading (SBUS) and writing (Sabertooth)
    sbus = fopen(port_name_1, "rb");
    sabertooth = fopen(port_name_2, "wb");
